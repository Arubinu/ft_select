...
/*   Updated: 2014/01/12 16:46:26 by apergens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			main(int argc, char **argv)
{
	...
	ft_putendl_fd("ft_select: Loading ... Please wait !", STDERR_FILENO);
	search = NULL;
...
int			ft_select(t_choice **ch, t_search **s, int *argc, int search)
{
	...
	while (ft_st_cmdgoto(0, 0) && (c = ft_getch()) && (key = ft_st_getkey(c)))
	{
		...
		if (!search && (key == GKEY_ESC || key == GKEY_RTN))
			break ;
		else if (!search && key == GKEY_ESP && ft_st_sound(key))
		{
			(*ch + pos)->check = ((*ch + pos)->check + 1) % 2;
			pos++;
		}
...
static int	ft_select_search(t_choice **ch, t_search **s, char *c, int *pos)
{
	...
	if (!(ret = 0) && ((key = ft_st_getkey(c)) == GKEY_UP || key == GKEY_STAR
